...
router.post('/create', wrapAsync(async (req: any, res: any) => {
    const createdRow = await prisma.$transaction(async (tx) => {
        const createdRow = await tx.trainer_team.create({ data: req.body });
        return createdRow;
    });
...
router.post('/read', wrapAsync(async (req: any, res: any) => {
    const response = await prisma.$transaction(async (tx) => {
        const rows = await tx.trainer_team.findMany({
            skip: req.body?.skip,
            take: req.body?.take || 1,
            where: req.body?.where,
            orderBy: req.body?.orderBy || { id: 'desc' },
        });
        const count = await tx.trainer_team.count({ where: req.body?.where });
        return {
            rows: rows,
            count: count,
        };
...
router.post('/update', wrapAsync(async (req: any, res: any) => {
    const updatedRow = await prisma.$transaction(async (tx) => {
        const updatedRow = await tx.trainer_team.update({ where: { id: req.body.id }, data: req.body });
        return updatedRow;
    });
...
router.post('/delete', wrapAsync(async (req: any, res: any) => {
    const deletedRow = await prisma.$transaction(async (tx) => {
        const deletedRow = await tx.trainer_team.delete({ where: { id: req.body.id } });
        return deletedRow;
    });
